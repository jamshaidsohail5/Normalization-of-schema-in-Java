	...
	private void calculateSecondNormalForm() {
		...
		if (failedAttrs.isEmpty()) {
			...
			for (int i = 0; i < failedAttrs.size(); i++) {
				...
				sb.append("}\nwhereas it should only be functionally determined by the full set of attributes of the composite minimum key {");
	...
	private void calculateThirdNormalForm() {
		...
		if (failedFDs.isEmpty() && isSecondNormalForm) {
			...
			if (failedFDs.isEmpty()) {
				twoNFStatusEnding = ".";
				threeNFStatus = " Otherwise it would be in 3NF as all of the relation's functional dependencies satisfy at least one of the following conditions:\n " + threeNFRequirements;
			} else {
