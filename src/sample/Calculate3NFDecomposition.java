...
public class Calculate3NFDecomposition extends CalculateDecomposition {
	...
	private void decompose(final boolean force3NFDecomposition) {
		...
		appendOutputMsg("\nFor each functional dependency of the canonical cover set (merging functional dependencies having the same left-hand attribute(s)) " +
				"\nof original relation's functional dependencies, "
		...
		if (originalAttributes.size() > addedAttributes.size()) {
			appendOutputMsg("\nThere is at least one attribute from original relation that has not been placed in any new relation, "
		...
		appendOutputMsg("\nChecking if at least one key can be found in at least one newly formed 3NF relation.");
		...
		if (foundKey != null) {
			appendOutputMsg("\nSince key {" + foundKey.printLeftSideAttributes() +
					"} is present in at least one of the new 3NF relations, no new relation was created.");
		} else {
			appendOutputMsg("\nSince none of the newly created 3NF relations contains a key of the original relation, need to "
			...
			appendOutputMsg("\nAdded key {" + getInputRelation().getMinimumKeyClosures().get(0).printLeftSideAttributes() + "}. ");
		...
		appendOutputMsg("\nTesting if any relation includes all of the attributes found in another relation "
		...
		if (removedone) {
			appendOutputMsg("\nRemoved at least one new relation that was a duplicate or subset of another new relation.");
		} else {
			appendOutputMsg("\nNo new relations were removed.");
		}
		for (int i = 0; i < workingOutputRelations.size(); i++) {
			if (!removeIndices[i]) {
